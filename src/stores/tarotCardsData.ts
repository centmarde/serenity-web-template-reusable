...
import { calculateEndDate } from '../pages/tarotCards/utils/helpers';
...
interface TarotCardsDataState {
  ...
  // CRUD Actions
  createDeck: (deck: CreateTarotCardsDeckInput) => Promise<TarotCardsDeck | null>;
  getDeck: (id: number) => Promise<TarotCardsDeck | null>;
  getAllDecks: () => Promise<TarotCardsDeck[]>;
  updateDeck: (deck: UpdateTarotCardsDeckInput) => Promise<TarotCardsDeck | null>;
  ...
  saveFromAiReading: (session: TarotReadingSession, isGf?: boolean) => Promise<TarotCardsDeck | null>;
}

export const useTarotCardsDataStore = create<TarotCardsDataState>((set, get) => ({
  // Initial State
  decks: [],
  currentDeck: null,
  isLoading: false,
  ...
  getDeck: async (id: number) => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to get deck';
  ...
  getAllDecks: async () => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to get decks';
  ...
  updateDeck: async (deckData: UpdateTarotCardsDeckInput) => {
    ...
    try {
      ...
      if (deckData.end_date !== undefined) updateData.end_date = deckData.end_date;
      ...
      set({ 
        decks: updatedDecks,
        currentDeck: get().currentDeck?.id === deckData.id ? data : get().currentDeck,
        isLoading: false 
      });
  ...
  getMyDecks: async () => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to get my decks';
  ...
  getGfDecks: async () => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to get girlfriend decks';
  ...
  getRecentDecks: async (limit: number = 10) => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to get recent decks';
  ...
  // AI Reading Integration
  saveFromAiReading: async (session: TarotReadingSession, isGf: boolean = false) => {
    set({ isLoading: true, error: null });
    ...
    try {
      ...
      console.log(`🔮 Saved AI reading session ${session.sessionId} to database:`, result);
      
      return result;
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Failed to save AI reading to database';
...
export const useTarotCardsHelpers = () => {
  ...
  return {
    ...
    saveAiReadingToDatabase: async (session: TarotReadingSession, isGf?: boolean) => {
    ...
    isCompleteReading: (deck: TarotCardsDeck): boolean => {
      return !!(deck.card1 && deck.card2 && deck.card3 && 
                deck.card4 && deck.card5 && deck.card6);
    },
    ...
    formatDeckForDisplay: (deck: TarotCardsDeck) => ({
      ...
      time: new Date(deck.created_at).toLocaleTimeString(),
      endDate: deck.end_date ? new Date(deck.end_date).toLocaleDateString() : null,
      isGf: deck.is_gf,
      cardCount: [deck.card1, deck.card2, deck.card3, deck.card4, deck.card5, deck.card6]
        .filter(Boolean).length,
      isComplete: !!(deck.card1 && deck.card2 && deck.card3 && 
                     deck.card4 && deck.card5 && deck.card6),
