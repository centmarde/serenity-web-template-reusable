 ...
 */
export function calculateMonthsBetween(startDate: string | Date, endDate: Date = new Date()): number {
 ...
 */
export function calculateYearsBetween(startDate: string | Date, endDate: Date = new Date()): number {
  const start = new Date(startDate);
  const end = new Date(endDate);
 ...
 */
export function getFallbackComfortMessage(userName: string, partnerName: string): {
  title: string;
  content: string;
